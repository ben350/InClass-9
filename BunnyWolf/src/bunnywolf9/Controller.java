...
/**
 * Set up the GUI and handles events, and lets the bunny and
 * wolf take turns moving.
 */
public class Controller extends Frame implements Runnable {
    ...
    private Button resetButton = new Button("New");
    ...
    private void step() {
        ...
        if (model.gameIsOver) {
            if (model.bunnyIsAlive)
                messageLabel.setText("THE BUNNY HAS ESCAPED!");
            else
                messageLabel.setText("THE WOLF EATS THE BUNNY AFTER " +
    ...
    class StepButtonHandler implements ActionListener {
         ...
         * bunny hunt is advanced one step. While single stepping,
